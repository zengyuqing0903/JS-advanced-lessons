...
// 扩展运算符背后调用的是遍历接口（System.iterator），如果一个对象没有部署这个接口，就无法转换。Array.from方法则是支持类似数组的对
...
// fill方法还可以接受第二个和第三参数，用于指定填充的起始位置和结束位置
['x','y','z'].fill(7,1,2);//['x',7,'z']
// 上面的代码表示，fill方法从1号位开始，向原数组填充7，到2号位前结束

// ESS6提供三个新的方法——entries()，keys()和values()——用于遍历数组。
...
/*
// 下述内容参见迭代器章节
// 如果不使用for...of循环，可以手动调用遍历对象的next方法，进行遍历。iterator参见后续章节
let letter = ['a','b','c'];
...
new Array(3);//[ , ,]
// 空位不是undefined，一个位置的值等于undefined，依然是有值的。空位是没有任何值，in运算可以说明这一点
