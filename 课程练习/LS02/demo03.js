...
(function(){
    ...
    var str_a = "a";
    var str_b = str_a;//原始数据类型直接访问值，是值赋值
    ...
    obj_b = {v:"c"};//注意：因为改的是整个对象，这里会在堆内存中创建一个新的对象值:{v:"c"},
    //而现在的obj_b引用的是这个对象，所以这里打印的obj_a依旧是{v:"b"}，而obj_b是{v:"c"}(两者在内存中引用的是不同对象了)
    console.log(obj_a,obj_b);
}());
...
console.log(obj_c.x1 === obj_d.x2);//true (基本类型，值与类型相同)
console.log(obj_c === obj_d);//false (所指的内存空间不同)

console.log({m:1}==={m:1});//false (所指的内存空间不同)
